...
Training_Data_PATH = '/tf/raw_packets/http2_preprocessed_split/'
...
def generator_wrapper(generator):
    for data in generator:
        yield (
            (
                tf.convert_to_tensor(data[0][0], dtype=tf.float32),
                tf.convert_to_tensor(data[0][1], dtype=tf.float32),
                tf.convert_to_tensor(data[0][2], dtype=tf.float32),
            ),
...
train_dataset = tf.data.Dataset.from_generator(
    lambda: generator_wrapper(gen_hard.next_train()),
...
for epoch in range(nb_epochs):
    ...
    model_triplet.fit(train_dataset,
